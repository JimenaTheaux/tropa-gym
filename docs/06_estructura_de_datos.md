...
- `generar_cargos_periodo(periodo)`: calcula y crea cargos, requiere preview antes de confirmar. Reglas:
  ...
  - **Precio (RN-030)**: sale de `precios.combo_id`. El combo a usar se resuelve así, en orden:
    ...
    4. (Migración 20) Si ese **último pago** anterior tuvo `descuento_id` no nulo, el cargo nuevo también se genera con `monto = 0` y `monto_definido = false`, aunque el combo sí tenga precio vigente — un descuento de un mes no se reaplica solo al siguiente, así que se fuerza el mismo circuito de "a definir" para que Admin/Profesor decida el monto real.
