...
// historial de pagos del alumno en ese período (fecha, monto, forma de pago)
// para poder validar de un vistazo si el cargo pendiente ya está cubierto.
export function PagosPeriodoDrawer({
  ...
  const pagosQuery = useQuery({
    queryKey: queryKeys.pagosAlumnoPeriodo(alumnoId ?? '', periodo),
  ...
  return (
    <Drawer open={!!alumnoId} title={`Pagos — ${alumnoNombre}`} onClose={onClose}>
      <div className="flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-outline-variant bg-surface-container-low px-4 py-3">
          <div>
            ...
            <p className="font-inter text-sm text-on-surface">
              Cargo {money(cargoMonto)} · Pagado {money(totalPagado)}
            </p>
          </div>
          <BadgeEstadoCargo estado={cargoEstado} />
        </div>
        ...
        {!pagosQuery.isFetching && pagos.length === 0 && (
          <p className="font-inter text-sm text-on-surface-variant">Sin pagos registrados en este período.</p>
        )}
              ...
              <tbody>
                {pagos.map((p) => (
                  <tr key={p.id} className="border-t border-outline-variant">
                    ...
                    <td className="px-3 py-2 font-inter text-sm text-on-surface">{formatFecha(p.fecha.slice(0, 10))}</td>
